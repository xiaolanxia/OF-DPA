...
#define OF_MATCH_V1_INCOMPAT ( (uint64_t)0 \
    ...
    | ((uint64_t)1 << OF_OXM_INDEX_OFDPA_BFD_DISCRIMINATOR)\
...
of_match_more_specific(of_match_t *entry, of_match_t *query)
{
    ...
    /* Mask and values for ofdpa_bfd_discriminator */
    if (!OF_MORE_SPECIFIC_INT(e_m->ofdpa_bfd_discriminator, q_m->ofdpa_bfd_discriminator)) {
        return 0;
    }
    if (!OF_RESTRICTED_MATCH_INT(e_f->ofdpa_bfd_discriminator, q_f->ofdpa_bfd_discriminator,
            q_m->ofdpa_bfd_discriminator)) {
        return 0;
    }
...
of_match_overlap(of_match_t *match1, of_match_t *match2)
{
    ...
    /* Check overlap for ofdpa_bfd_discriminator */
    if (!OF_OVERLAP_INT(f1->ofdpa_bfd_discriminator, f2->ofdpa_bfd_discriminator,
        m2->ofdpa_bfd_discriminator, m1->ofdpa_bfd_discriminator)) {
        return 0; /* This field differentiates; all done */
    }
