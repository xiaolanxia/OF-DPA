                                  ...
                                  uint64_t *group_action_sf_bitmap)
{
    ...
    OF_LIST_ACTION_ITER(actions, &act, rv) {
        ...
        case OF_ACTION_OFDPA_SET_MPLS_TC_FROM_TUNNEL_TABLE:
        {
            ...
            group_bucket->mplsEXPRemarkTableIndex = qosIndex;
            *group_action_bitmap |= IND_OFDPA_ACT_SET_MPLS_TC_FROM_TUNNEL_TABLE;
            break;
        }
        case OF_ACTION_OFDPA_SET_MPLS_PCPDEI_FROM_TUNNEL_TABLE:
        {
            uint8_t qosIndex;
            of_action_ofdpa_set_mpls_pcpdei_from_tunnel_table_qos_index_get(&act.ofdpa_set_pcpdei_from_table, &qosIndex);
            group_bucket->mplsEXPRemarkTableIndex = qosIndex;
        ...
        {
            uint32_t index;
            of_action_ofdpa_color_based_tx_count_index_get(&act.ofdpa_color_based_tx_count, &index);
            //group_bucket-> ofdpa_color_based_tx_count = index;
            *group_action_bitmap |= IND_OFDPA_ACT_COLOR_BASED_TX_COUNT;
            break;
        }
